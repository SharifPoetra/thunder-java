...
public class KissCommand extends InteractionCommand {
  ...
  private String text;
  private String[] msg = {"! Cute!", "!! UwU", "!!! Uh that's nice~"};

  public KissCommand(Thunder thunder) {
    this.thunder = thunder;
    this.name = "kiss";
    this.help = "Kisses the specified user.";
  ...
  public void execute(Object[] args, MessageReceivedEvent event) {
    try {
      ...
      byte[] data = NetworkUtil.download("https://emilia-api.xyz/api/kiss", "Bearer " + thunder.getConfig().getEmiliaKey());
