...
public class VolumeCommand extends MusicCommand {
  ...
  public void doCommand(CommandEvent event) {
    ...
    // event.reply("Sorry! the volume command are currently disabled due to still buggy, Thanks for
    // your understanding.");
    // return;
    ...
    if (event.getArgs().isEmpty()) {
      ...
      if (nvolume < 0 || nvolume > 150)
        event.reply(
            event.getClient().getError() + " Volume must be a valid integer between 0 and 150!");
      else {
