...
public class BatSlapCommand extends FunCommand {
  ...
  public void execute(Object[] args, MessageReceivedEvent event) {
    try {
      ...
      byte[] data = NetworkUtil.download("https://emilia-api.xyz/api/batslap?slapper=" + event.getAuthor().getEffectiveAvatarUrl() + "&slapped=" + user.getEffectiveAvatarUrl(), "Bearer " + thunder.getConfig().getEmiliaKey());
