...
public class PlaylistCommand extends OwnerCommand {
    ...
    public void execute(CommandEvent event) {
        StringBuilder builder =
                new StringBuilder(
                        event.getClient().getWarning() + " Playlist Management Commands:\n");
    ...
    public class MakelistCommand extends OwnerCommand {
        ...
        protected void execute(CommandEvent event) {
            ...
            if (thunder.getPlaylistLoader().getPlaylist(pname) == null) {
                ...
                event.reply(
                        event.getClient().getError() + " Playlist `" + pname + "` already exists!");
        }
    }
    ...
    public class DeletelistCommand extends OwnerCommand {
        ...
        protected void execute(CommandEvent event) {
            ...
            if (thunder.getPlaylistLoader().getPlaylist(pname) == null)
                event.reply(
                        event.getClient().getError() + " Playlist `" + pname + "` doesn't exist!");
            else {
                try {
    ...
    public class AppendlistCommand extends OwnerCommand {
        ...
        protected void execute(CommandEvent event) {
            ...
            if (parts.length < 2) {
                event.reply(
                        event.getClient().getError()
                                + " Please include a playlist name and URLs to add!");
                return;
            }
    ...
    public class ListCommand extends OwnerCommand {
        ...
        protected void execute(CommandEvent event) {
            ...
            if (list == null)
                ...
                event.reply(
                        event.getClient().getWarning()
                                + " There are no playlists in the Playlists folder!");
            else {
                StringBuilder builder =
                        new StringBuilder(
                                event.getClient().getSuccess() + " Available playlists:\n");
