...
public class AudioHandler extends AudioEventAdapter implements AudioSendHandler {
  ...
  public void setNightcore(float speed) {
    if (speed <= 0.1 || speed > 3)
      throw new IllegalArgumentException("Nightcore speed out of range (0.1-3)!"); // prevent zero division and other weird shit
  ...
  private List<AudioFilter> buildChain(AudioTrack audioTrack, AudioDataFormat format, UniversalPcmAudioFilter downstream) {
    ...
    if (canChangeSpeed(getPlayingTrack()) && (vaporwave || nightcore != 1 || tempo != 1 || pitch != 0)) {
      if (nightcore != 1 && highQualityNightcore) {
        filter = new ResamplingPcmAudioFilter(configuration, format.channelCount, filter,
    ...
    if (karaoke) {
      filter = new KaraokePcmAudioFilter(filter, format.channelCount, format.sampleRate)
